...
const INTENTS = {
  ...
  diet: ["diet","meal","food","eat","nutrition","what to eat","eating plan","meal plan","food plan","diet plan","diet chart","what should i eat","meal chart","daily food","food guide","recipes","grocery","what food","not vegan","not vegetarian","i eat meat","i eat chicken","halal","change diet","update diet","wrong diet","incorrect diet","my diet","food chart","eating chart","nutrition chart","what should i eat","daily meal","food plan","change my diet","update my diet"],
...
`
  ...
  "\n\nCheck the number or WhatsApp us: +8801935962421"
    ...
    // BMI / diet / calorie — if profile complete show stats, else start profile builder
    if ((has(msg, "bmi") || msg.includes("calculate") && msg.includes("bmi") || msg.includes("my bmi") || msg.includes("check bmi")) && !p.step) {
...
if (p.step) {
  ...
  return "Perfect. Your profile is saved.\n\nYou can now ask for:\n• Workout plan\n• Diet chart\n• Supplements\n• BMI stats\n\nWhat do you want next? 💪"
    ...
    const hasDietChange = msg.includes("change") || msg.includes("update") || msg.includes("switch") || msg.includes("different")
    const hasDietWord = msg.includes("diet") || msg.includes("food") || msg.includes("eat")
    const hasReligionWord = msg.includes("muslim") || msg.includes("halal") || msg.includes("hindu") ||
      msg.includes("vegan") || msg.includes("vegetarian") || msg.includes("vegeterian") ||
      msg.includes("veggie") || msg.includes("vegiterian") || msg.includes("non veg") || msg.includes("nonveg")
    const isReligionMsg = modeRef.current !== "awaiting_delivery_city" && (
      ((msg.includes("i am ") || msg.includes("im ") || msg.includes("i m ") || msg.includes("i'm ")) && hasReligionWord) ||
      ...
      (hasDietChange && hasReligionWord) ||
      (hasDietChange && hasDietWord && !msg.includes("workout") && !msg.includes("plan"))
    )
    if (isReligionMsg) {
      let newRel = p.religion || "none"
      if (msg.includes("muslim") || msg.includes("halal")) newRel = "muslim"
      else if (msg.includes("hindu")) newRel = "hindu"
      else if ((msg.includes("vegan") || msg.includes("vegen")) && !msg.includes("not") && !msg.includes("vegetarian") && !msg.includes("vegeterian")) newRel = "vegan"
      else if (msg.includes("vegetarian") || msg.includes("vegeterian") || msg.includes("veggie") || msg.includes("vegiterian")) newRel = "vegetarian"
      else if (msg.includes("not vegan") || msg.includes("not vegetarian") || msg.includes("i eat meat") || msg.includes("i eat chicken") || msg.includes("non veg")) newRel = "none"
      else {
        // "change my diet" with no religion specified — ask
        modeRef.current = "change_diet"
        return "What\'s your diet preference? 🥗\n\n1 — Muslim (Halal)\n2 — Hindu (no beef)\n3 — Vegetarian\n4 — Vegan\n5 — None (eat everything)"
      }
      setProfile({ ...p, religion: newRel })
      const dietName = newRel === "muslim" ? "Halal" : newRel === "hindu" ? "Hindu" : newRel === "vegetarian" ? "Vegetarian" : newRel === "vegan" ? "Vegan" : "Regular"
      return "Diet updated to " + dietName + "! ✅\nSay \'diet\' for your new personalized plan."
    }

    // Handle diet preference selection when in change_diet mode
    if (modeRef.current === "change_diet") {
      ...
      const chosen = dietMap[msg.trim()] ||
        (msg.includes("muslim") || msg.includes("halal") ? "muslim" :
         msg.includes("hindu") ? "hindu" :
         ...
         msg.includes("none") || msg.includes("everything") || msg.includes("all") || msg.includes("5") ? "none" : null)
      if (chosen) {
        modeRef.current = null
        setProfile({ ...p, religion: chosen })
        const dietName = chosen === "muslim" ? "Halal" : chosen === "hindu" ? "Hindu" : chosen === "vegetarian" ? "Vegetarian" : chosen === "vegan" ? "Vegan" : "Regular"
        return "Diet updated to " + dietName + "! ✅\nSay \'diet\' for your new personalized plan."
      }
      return "Please choose 1-5:\n1 — Muslim (Halal)\n2 — Hindu (no beef)\n3 — Vegetarian\n4 — Vegan\n5 — None"
    }
...
if (
  ...
  return getSupplements(p.goal || "general", isVeg)
    ...
    // Diet
    if (has(msg, "diet")) {
      // If asking to change diet specifically
      if ((msg.includes("change") || msg.includes("update") || msg.includes("switch") || msg.includes("different")) && !hasReligionWord) {
        modeRef.current = "change_diet"
        return "What\'s your diet preference? 🥗\n\n1 — Muslim (Halal)\n2 — Hindu (no beef)\n3 — Vegetarian\n4 — Vegan\n5 — None (eat everything)"
      }
      ...
      setProfile({ ...p, step: "height" })
      return "I\'ll build your personalized diet plan! 🥗\nFirst, what\'s your height in cm? (e.g. 175)"
    }
