...
export default function ChatBot() {
  ...
  useEffect(() => {
    ...
    if (messages.length <= DEFAULT_CHATBOT_MESSAGES.length) return
    saveChat(messages)
  }, [messages])
  ...
  useEffect(() => {
    async function load() {
      const [settingsRes, productsRes] = await Promise.all([
        ...
        fetch("/api/products?limit=20&in_stock=true").then(r => r.ok ? r.json() : {products:[]}).catch(() => ({products:[]}))
      ])
      const map: Settings = {}
      ...
      if (productsRes.products) productsCacheRef.current = productsRes.products
    }
    load()
  }, [])
      ...
      {open && (
        <div className="cwin" data-chatbox="true" style={{position:"fixed",bottom:"1.5rem",right:"2rem",width:"340px",height:"540px",backgroundColor:"white",border:"1px solid #e0e0e0",boxShadow:"0 20px 60px rgba(0,0,0,0.2)",zIndex:9998,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{background:"black",padding:"0.875rem 1rem",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:"0.65rem"}}>
              <div style={{position:"relative",width:"50px"}}>
                ...
                >
                  ...
                  <path d="M48 48 Q60 56 72 48" stroke="#00eaff" strokeWidth="2" strokeLinecap="round"/>
            ...
            <button onClick={()=>setOpen(false)} style={{background:"none",border:"none",color:"rgba(255,255,255,0.5)",fontSize:"1.2rem",cursor:"pointer",padding:"0.2rem",lineHeight:1}}>✕</button>
          </div>
          ...
          <div className="msgs" style={{flex:1,overflowY:"auto",padding:"0.875rem",display:"flex",flexDirection:"column",gap:"0.6rem"}}>
            {messages.map((msg,i)=>(
              <div key={i} className="cmsg" style={{display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start",alignItems:"flex-end",gap:"0.35rem"}}>
                {msg.role==="assistant"&&(
                  <svg className="robotGlow" width="26" height="30" viewBox="0 0 120 120" style={{flexShrink:0}}>
                      <rect x="25" y="15" width="70" height="45" rx="12" fill="#0b1625"/>
          ...
          {messages.length<=1&&(
            <div style={{padding:"0 0.875rem 0.6rem",display:"flex",flexWrap:"wrap",gap:"0.35rem"}}>
              {quick.map(q=>(
                <button key={q} className="qbtn" onClick={()=>sendMessage(q)} style={{fontSize:"0.66rem",padding:"0.28rem 0.65rem",border:"1px solid #e0e0e0",backgroundColor:"white",borderRadius:"20px",cursor:"pointer",color:"#444",fontFamily:"inherit",transition:"background 0.15s"}}>{q}</button>
              ))}
            </div>
          )}
          <div style={{padding:"0.6rem 0.875rem",borderTop:"1px solid #f0f0f0",display:"flex",gap:"0.4rem",flexShrink:0}}>
            ...
            <button onClick={()=>sendMessage()} disabled={!input.trim()||loading} style={{width:"38px",height:"38px",borderRadius:"50%",backgroundColor:input.trim()?"black":"#ddd",border:"none",color:"white",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.2s"}}>
